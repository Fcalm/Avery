...
const liveRequested = process.env.AVERY_DEEPSEEK_LIVE === '1';
...
describe.skipIf(!liveRequested)('DeepSeek real usage integration', () => {
  it('真实响应返回完整且自洽的 Provider usage', async () => {
    ...
    expect(apiKey, 'AVERY_DEEPSEEK_LIVE=1 时必须提供 DEEPSEEK_API_KEY').toBeTruthy();
    const model = process.env.AVERY_DEEPSEEK_MODEL?.trim() || 'deepseek-v4-flash';
