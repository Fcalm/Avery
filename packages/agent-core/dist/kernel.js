...
function HistorySnapshot(transcript) {
    return (0, agent_sdk_1.KeepRecentTurnGroups)(transcript.filter((message) => message.role !== 'system'), 5);
}
/** 从工具结果解析统一 disposition；优先读取结构化字段，其次兼容旧 payload 标记。 */
function ParseToolDisposition(result) {
    if (result.disposition && result.disposition !== 'continue')
        return result.disposition;
    try {
        const payload = JSON.parse(result.content);
        if (payload.awaitingUser === true)
            return 'wait_user_input';
        if (payload.code === 'CONFIRMATION_REQUIRED')
            return 'wait_confirmation';
        if (payload.code === 'STATUS_UNKNOWN' || payload.code === 'PAUSED')
            return 'pause';
    }
...
function CreateSkippedResult(call) {
    return {
        role: 'tool',
...
async function RunToolBatch(input, calls) {
    ...
    for (const phase of phases) {
        ...
        for (const { call, result } of phaseResults) {
            ...
            if (disposition === 'continue') {
                if (callDisposition === 'wait_user_input')
                    disposition = 'waiting_user_input';
                else if (callDisposition === 'wait_confirmation')
                    disposition = 'waiting_confirmation';
                else if (callDisposition === 'pause')
                    disposition = 'paused';
            }
        }
        ...
        if (disposition !== 'continue') {
            for (const call of calls.slice(executedCount)) {
                const index = calls.findIndex((item) => item.id === call.id);
                if (index >= 0)
                    results[index] = CreateSkippedResult(call);
            }
        }
    }
    for (let index = 0; index < calls.length; index += 1) {
        if (!results[index])
            results[index] = CreateSkippedResult(calls[index]);
    }
