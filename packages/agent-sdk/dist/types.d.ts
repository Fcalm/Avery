...
export type RunState = 'created' | 'preparing' | 'model_streaming' | 'tool_validating' | 'tools_running' | 'waiting_user_input' | 'waiting_confirmation' | 'paused' | 'completed' | 'failed' | 'cancelled';
/** 工具批次执行后的统一运行去向：普通继续、等待用户、等待确认或暂停。 */
export type RunDisposition = 'continue' | 'waiting_user_input' | 'waiting_confirmation' | 'paused' | 'completed' | 'failed' | 'cancelled';
