...
export interface ModelProviderModule extends ModuleManifest {
  slot: 'model-provider';
  Configure(input: unknown): Promise<{ configured: boolean; provider: string; model: string }>;
...
export interface CompactionModule extends ModuleManifest {
  ...
  SplitRecentTurns(history: AgentMessage[]): { earlier: AgentMessage[]; recent: AgentMessage[] };
...
export interface InteractionModule extends ModuleManifest {
  ...
  ConfirmResumeEdit(confirmationId: string, accepted: boolean, context: ToolContext): Promise<{ applied: boolean }>;
...
} as const satisfies Record<SlotName, keyof AgentModules>;
