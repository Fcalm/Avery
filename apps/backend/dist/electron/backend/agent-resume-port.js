...
class AgentResumePort {
    ...
    async ReadCurrent(resumeId) {
        const viewModel = await this.business.LoadViewModel();
        return viewModel.resumes.find((resume) => resume.id === resumeId) ?? null;
    }
    /** 尝试获取简历互斥锁；被其他 owner 占用时返回未获取与错误码。 */
    async AcquireLock({ resumeId, owner, ownerId, baseRevision }) {
        const lock = this.lockStore.Acquire(resumeId, owner, ownerId, baseRevision);
        if (!lock)
            return { acquired: false, code: owner === 'user' ? 'RESOURCE_LOCKED' : 'RESUME_LOCKED_BY_USER' };
    ...
    async Save({ resume, baseRevision }) {
        try {
