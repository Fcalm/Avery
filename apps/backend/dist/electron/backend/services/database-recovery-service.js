...
const MigrationManifest = require('../../../../../../migrations/business/manifest.json');
const CoreTables = ['workspace_meta', 'schema_migrations', 'conversations', 'attachments'];
function ExpectedChecksums() {
    return new Map(MigrationManifest.migrations.map((entry) => [entry.version, crypto.createHash('sha256').update(entry.checksumSeed).digest('hex')]));
}
...
function ValidateBackupManifest(manifestPath, databasePath, profilePath) {
    ...
    // 旧日备份只记录文件名；带摘要的新格式必须逐字节校验，不能把被篡改的升级备份标为可恢复。
    ...
    if (manifest?.profile && typeof manifest.profile === 'object') {
        if (manifest.profile.file !== 'profile.json' || !fs.existsSync(profilePath) || !/^[a-f0-9]{64}$/.test(manifest.profile.sha256 || '') || FileSha256(profilePath) !== manifest.profile.sha256) {
...
class DatabaseRecoveryStore {
    constructor({ workspacePath, cause }) {
        ...
        fs.mkdirSync(path.join(this.workspacePath, 'backups'), { recursive: true });
        fs.mkdirSync(path.join(this.workspacePath, 'exports'), { recursive: true });
    ...
    ListBackups() {
        ...
        return fs.readdirSync(root, { withFileTypes: true }).filter((entry) => entry.isDirectory() && !entry.isSymbolicLink() && entry.name.startsWith('daily-'))
            .map((entry) => {
            ...
            try {
                ...
                return { id: entry.name, valid: true, schemaVersion: validation.schemaVersion, createdAt: fs.statSync(directory).mtimeMs };
            }
            catch {
                return { id: entry.name, valid: false, schemaVersion: null, createdAt: fs.statSync(directory).mtimeMs };
            }
    ...
    RestoreBackup(backupId) {
        ...
        const backupStat = fs.lstatSync(backupDirectory, { throwIfNoEntry: false });
        if (!backupStat || !backupStat.isDirectory() || backupStat.isSymbolicLink())
            throw new Error('Backup directory is unsafe.');
        const backupReal = fs.realpathSync(backupDirectory);
        if (path.dirname(backupReal) !== fs.realpathSync(backupRoot))
        ...
        try {
            fs.copyFileSync(path.join(backupDirectory, 'offerget.db'), path.join(staging, 'offerget.db'));
            if (fs.existsSync(path.join(backupDirectory, 'profile.json')))
                fs.copyFileSync(path.join(backupDirectory, 'profile.json'), path.join(staging, 'profile.json'));
            ...
            for (const name of ['offerget.db', 'offerget.db-wal', 'offerget.db-shm', 'profile.json']) {
                ...
                if (fs.existsSync(current))
                    fs.renameSync(current, path.join(scene, name));
            }
            originalsMoved = true;
            fs.renameSync(path.join(staging, 'offerget.db'), this.databasePath);
            if (fs.existsSync(path.join(staging, 'profile.json')))
                fs.renameSync(path.join(staging, 'profile.json'), this.profilePath);
            fs.writeFileSync(path.join(scene, 'diagnostic.json'), JSON.stringify({ reason: this.reason, restoredFrom: backupId, sceneId: path.basename(scene) }, null, 2), 'utf8');
            fs.rmdirSync(staging);
            ValidateRecoverySet(this.databasePath, this.profilePath);
            return { restored: true, backupId, sceneId: path.basename(scene) };
        }
        catch (error) {
            // 成组回滚：即使新 DB 已安装而 profile 安装失败，也先隔离新文件，再恢复全部原现场，禁止混合版本。
            if (originalsMoved) {
                for (const name of ['offerget.db', 'offerget.db-wal', 'offerget.db-shm', 'profile.json']) {
                    const current = path.join(this.workspacePath, name);
                    const original = path.join(scene, name);
                    if (fs.existsSync(current))
                        fs.renameSync(current, path.join(scene, `failed-recovery-${name}`));
                    if (fs.existsSync(original))
                        fs.renameSync(original, current);
                }
            }
    ...
    ExportRecoveryDiagnostic() {
        ...
        fs.writeFileSync(path.join(this.workspacePath, 'exports', fileName), JSON.stringify({ reason: this.reason, backups: this.ListBackups().map(({ id, valid, schemaVersion }) => ({ id, valid, schemaVersion })) }, null, 2), 'utf8');
        return { exported: true, fileName };
    }
    Close() { }
}
module.exports = { DatabaseRecoveryStore, ValidateRecoverySet };
