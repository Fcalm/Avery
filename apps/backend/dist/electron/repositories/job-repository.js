...
class JobRepository {
    ...
    ListAll() {
        return this.db.prepare('SELECT payload_json, revision FROM jobs WHERE deleted_at IS NULL ORDER BY updated_at DESC').all()
            .map((row) => (0, enum_map_1.JobToDisplay)({ ...JSON.parse(row.payload_json), revision: row.revision }));
    }
    ...
    Upsert(job, expectedRevision) {
        ...
        const storageJob = (0, enum_map_1.JobToStorage)(job);
        const now = (0, helpers_1.GetNow)();
        const existing = this.db.prepare('SELECT revision FROM jobs WHERE id = ?').get(job.id);
        if (existing)
            (0, helpers_1.AssertRevision)(existing, expectedRevision, 'job', job.id);
        ...
        (0, helpers_1.WriteAudit)(this.db, 'user', 'save', 'job', job.id, {});
    ...
    SetFavorite(id, favorite, expectedRevision) {
        ...
        (0, helpers_1.AssertRevision)(existing, expectedRevision, 'job', id);
        const payload = JSON.parse(existing.payload_json);
        payload.favorite = Boolean(favorite);
        const nextRevision = existing.revision + 1;
        this.db.prepare('UPDATE jobs SET is_favorite = ?, payload_json = ?, revision = ?, updated_at = ? WHERE id = ?').run(favorite ? 1 : 0, JSON.stringify(payload), nextRevision, (0, helpers_1.GetNow)(), id);
        (0, helpers_1.WriteAudit)(this.db, 'user', favorite ? 'favorite' : 'unfavorite', 'job', id, {});
    ...
    Delete(id) {
        if (typeof id !== 'string' || id.length === 0 || id.length > 200)
            throw new Error('Job id is invalid.');
        this.db.prepare('UPDATE jobs SET deleted_at = ?, updated_at = ? WHERE id = ?').run((0, helpers_1.GetNow)(), (0, helpers_1.GetNow)(), id);
        (0, helpers_1.WriteAudit)(this.db, 'user', 'delete', 'job', id, {});
        return { id };
    }
}
