...
const { ApplicationToStorage, ApplicationToDisplay, ToStorage, ApplicationStatusStorage } = require('./enum-map.js');
...
class ApplicationRepository {
    ...
    ListAll() {
        return this.db.prepare('SELECT payload_json, revision FROM applications ORDER BY updated_at DESC').all()
            .map((row) => ApplicationToDisplay({ ...JSON.parse(row.payload_json), revision: row.revision }));
    }
    ...
    Upsert(application, expectedRevision) {
        ...
        if (existing)
            AssertRevision(existing, expectedRevision, 'application', application.id);
    ...
    MoveStatus(id, status, expectedRevision) {
        ...
        if (existing.status !== storageStatus) {
            ...
            WriteAudit(this.db, 'user', 'move_status', 'application', id, { from: existing.status, to: storageStatus });
