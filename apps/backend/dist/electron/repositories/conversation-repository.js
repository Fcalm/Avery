...
class ConversationRepository {
    ...
    Create({ id, title }) {
        ...
        (0, helpers_1.WriteAudit)(this.db, 'user', 'create', 'conversation', id, {});
    ...
    Rename(id, title, expectedRevision) {
        ...
        (0, helpers_1.AssertRevision)(existing, expectedRevision, 'conversation', id);
        const nextRevision = existing.revision + 1;
        const result = this.db.prepare('UPDATE conversations SET title = ?, revision = ?, updated_at = ?, last_used_at = ? WHERE id = ?').run(title, nextRevision, (0, helpers_1.GetNow)(), (0, helpers_1.GetNow)(), id);
        if (!result.changes)
            throw new Error('Conversation was not found.');
        (0, helpers_1.WriteAudit)(this.db, 'user', 'rename', 'conversation', id, {});
    ...
    Delete(id) {
        ...
        const run = this.db.transaction(() => {
            ...
            (0, helpers_1.WriteAudit)(this.db, 'user', 'delete', 'conversation', id, {});
        });
        run();
        return { id };
    }
    ...
    Touch(id) {
        if (typeof id !== 'string' || id.length === 0 || id.length > 200)
            throw new Error('Conversation id is invalid.');
        this.db.prepare('UPDATE conversations SET updated_at = ?, last_used_at = ? WHERE id = ?').run((0, helpers_1.GetNow)(), (0, helpers_1.GetNow)(), id);
    }
    ...
    CompleteMessage(conversationId, messageId, content, thinkingContent) {
        ...
        const result = this.db.prepare('UPDATE conversation_messages SET content = ?, reasoning_content = ?, status = ?, updated_at = ? WHERE id = ? AND conversation_id = ?')
            .run(content, thinkingContent ?? null, 'complete', (0, helpers_1.GetNow)(), messageId, conversationId);
        if (!result.changes)
    ...
    SetSessionSnapshot(conversationId, json) {
        ...
        this.db.prepare('UPDATE conversations SET session_snapshot_json = ?, updated_at = ? WHERE id = ?').run(json, (0, helpers_1.GetNow)(), conversationId);
    ...
    SetToolSnapshot(conversationId, json) {
        ...
        this.db.prepare('UPDATE conversations SET tool_array_snapshot_json = ?, updated_at = ? WHERE id = ?').run(json, (0, helpers_1.GetNow)(), conversationId);
    }
