...
const parentPort = (process as any).parentPort as { postMessage(message: unknown): void; once(event: string, listener: (...args: any[]) => void): unknown; on(event: string, listener: (...args: any[]) => void): unknown } | undefined;
const PostMessage = (message: unknown): void => parentPort?.postMessage(message);

// 子进程 stderr 不回流主进程控制台，统一经 parentPort 上报未捕获异常以便诊断；注册须早于后续 require。
process.on('uncaughtException', (error) => {
  try {
    PostMessage({ kind: 'backend-error', code: 'UNCAUGHT', message: String((error as any)?.stack || error) });
  } catch { /* 上报失败时保持默认退出。 */ }
});
process.on('unhandledRejection', (reason) => {
  try {
    PostMessage({ kind: 'backend-error', code: 'UNHANDLED', message: String((reason as any)?.stack || reason) });
  } catch { /* 上报失败时保持默认行为。 */ }
});

// 必须在依赖 pdf-parse 的模块 require 之前注入，否则其 bundle 顶层引用 DOMMatrix 会抛 ReferenceError。
const { InstallBrowserPolyfills } = require('./electron/backend/pdf-polyfills.js') as { InstallBrowserPolyfills(): void };
InstallBrowserPolyfills();

// 进程编排层：TS 模块（同包 require 保持调用时序，不强求类型）。
const { CreateWorkerHost } = require('./worker-host.js') as any;
const { CreateBackend, ReadOnlyChannels } = require('./router.js') as any;
...
async function Bootstrap(): Promise<void> {
  if (!parentPort) throw new Error('parentPort is unavailable in Backend process.');
  const hello = await new Promise<any>((resolve) => parentPort.once('message', (event: any) => resolve(event?.data ?? event)));
  const app = hello?.app ?? {};
  ...
  async function MigrateWorkspace(): Promise<any> {
    ...
    try {
      ...
      if (!destinationPath) return { ...(await host.business.GetStatus()), migration: { succeeded: false, cancelled: true } };
    ...
    get(target: any, property: string | symbol) {
      if (typeof property === 'string' && guardedMethods.has(property)) {
        return async (...args: any[]) => {
          ...
          if (lock && lock.owner === 'agent') throw Object.assign(new Error('Agent is currently editing this resume.'), { code: 'RESOURCE_LOCKED' });
  ...
  const Facade = (methods: string[]): any => {
    const service: Record<string, (...args: any[]) => any> = {};
    for (const method of methods) service[method] = (...args: any[]) => guardedBusiness[method](...args);
    return service;
  };
  ...
  const backend = CreateBackend({
    container: {
      ...
      desktop: {
        ...
        ExportResume: (resume: unknown, format: unknown) => desktop.Call('ExportResume', [{ workspacePath: currentWorkspacePath, resume, format }]),
      },
    },
...
Bootstrap().catch((error) => {
  ...
  PostMessage({ kind: 'backend-error', code: 'INTERNAL_ERROR', message: (error as Error)?.message || 'Backend bootstrap failed.' });
