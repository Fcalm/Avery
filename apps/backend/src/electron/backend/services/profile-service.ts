import { WriteAudit } from '../../repositories/helpers';
...
export class ProfileService {
  ...
  constructor({ repository, db, attachmentLifecycle, workspaceOperations }: { repository: any; db: any; attachmentLifecycle: any; workspaceOperations: any }) {
  ...
  Save(items: any[], force = false): any {
    ...
    try {
      ...
      if (force) WriteAudit(this.db, 'user', 'profile_keep', 'profile', 'profile.json', { from: before, to: result.hash });
  ...
  SynchronizeAttachmentLinks(items: any[]): void {
    const desired = new Set(items.map((item: any) => item.id));
    const existing = this.db.prepare("SELECT DISTINCT owner_id FROM attachment_links WHERE owner_type = 'profile'").all();
    const run = this.db.transaction(() => {
      for (const item of items) this.attachmentLifecycle.ReplaceLinks('profile', item.id, item);
      for (const row of existing) if (!desired.has(row.owner_id)) this.attachmentLifecycle.RemoveLinks('profile', row.owner_id);
    });
    run();
  }
}
