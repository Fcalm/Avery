...
export function NormalizeBrowserAddress(address: unknown): string | null {
  if (typeof address !== 'string' || !address.trim() || address.length > 2_048) return null;
...
export function NormalizeBrowserBounds(value: unknown, contentSize: readonly number[]): BrowserPanelBounds | null {
  if (!value || typeof value !== 'object') return null;
  const candidate = value as Partial<BrowserPanelBounds>;
  ...
  if (bounds.x < 0 || bounds.y < 0 || bounds.width < MinimumPanelSize || bounds.height < MinimumPanelSize) return null;
  const [contentWidth, contentHeight] = contentSize;
  if (!Number.isFinite(contentWidth) || !Number.isFinite(contentHeight)) return null;
  if (bounds.x + bounds.width > contentWidth || bounds.y + bounds.height > contentHeight) return null;
  return bounds;
}
