...
- 统一投递场景 Prompt、100 轮预算、22 个工具白名单、强制确认动作和完成证据。
- 修正文档中“投递场景暂未实现/不可创建 Run”与现状冲突的描述。
- 明确等待确认通过 `ConfirmBrowserAction` 执行原 proposal，后续继续任务必须从新 Snapshot 开始。

验收标准：

- [x] 场景快照包含 9 个共享只读/Todo/交互工具和 13 个浏览器工具，共 22 个；其中 `BrowserFillForm` 仅批量填写普通输入框。
...
| 部分 | 状态 | 证据/说明 |
| --- | --- | --- |
| AP-01 投递场景协议 | 已完成 | 已同步开发态/未发布状态；场景冻结 9 个共享工具和 13 个浏览器工具（12 个原子工具与 1 个受限批量输入工具） |
