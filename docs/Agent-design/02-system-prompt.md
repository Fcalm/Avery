...
| L2 Scenario | 简历优化、岗位定制、项目提炼、投递辅助的目标与边界 | 场景注册表 | 只能被更严格地收窄 |
...
- 可以帮助澄清、起草、优化、组织和规划。
- 普通 Agent 不负责自动化浏览器提交、导出文件或任意网络访问。
- 默认使用用户语言，编辑性判断可自主完成，事实性缺口必须显式处理。
