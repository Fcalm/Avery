...
import { ClearAgentObservability, ClearBrowserProfile, ConfigureAgent, GetAgentModuleConfiguration, GetAgentStatus, GetBrowserRuntimeStatus, GetDeepSeekModels, IsDesktopAgentAvailable, ResetAgentModules, SelectAgentModuleDirectory, SetAgentTraceRetention, TestAgentConnection } from '../../../features/assistant/api/agentQueries';
...
const Tabs: Array<{ id: SettingTab; label: string; description: string }> = [
  ...
  { id: 'browser', label: '浏览器设置', description: '管理 Avery 独立浏览器的运行状态与登录数据。' },
...
function IsValidContextLength(value: string): boolean {
  const match = value.trim().match(/^(\d+)\s*([kK])?$/);
  const limit = match ? Number(match[1]) * (match[2] ? 1000 : 1) : Number.NaN;
  return Number.isSafeInteger(limit) && limit >= 8_000 && limit <= 2_000_000;
}
...
function SettingsPage({ onNavigateDeveloper }: { onNavigateDeveloper: () => void }) {
  ...
  const [browserRuntime, setBrowserRuntime] = useState<AgentBrowserRuntimeStatus | null>(null);
  const [browserRuntimeBusy, setBrowserRuntimeBusy] = useState(false);
  const [showClearBrowserProfile, setShowClearBrowserProfile] = useState(false);
  ...
  async function TestApiConnection() {
    ...
    try {
      await TestAgentConnection({ provider: draft.provider, apiKey: draft.apiKey, baseUrl: draft.baseUrl, model: draft.model, thinkingEnabled: draft.thinkingEnabled, contextLength: draft.contextLength, contextLimitMode: draft.contextLimitMode, compressionThreshold: draft.compressionThreshold });
  ...
  async function RefreshBrowserRuntime() {
    try { setBrowserRuntime(await GetBrowserRuntimeStatus()); } catch { setBrowserRuntime(null); }
  }
  async function ClearBrowserProfileHandler() {
    setBrowserRuntimeBusy(true);
    try {
      await ClearBrowserProfile();
      setShowClearBrowserProfile(false);
      ShowNotice('独立浏览器登录数据已清除');
      await RefreshBrowserRuntime();
    } catch (error) { ShowNotice(error instanceof Error ? error.message : '浏览器登录数据清理失败'); }
    finally { setBrowserRuntimeBusy(false); }
  }
  ...
  return <div className="settings-page"><aside className="settings-nav"><div><p className="eyebrow">设置</p><h1>应用设置</h1></div>{Tabs.map((item) => <button key={item.id} className={tab === item.id ? 'selected' : ''} aria-current={tab === item.id ? 'page' : undefined} onClick={() => SwitchTab(item.id)}>{item.label}</button>)}</aside><section className="settings-content"><PageHeader eyebrow="设置" title={current.label} description={current.description} actions={tab !== 'api' && tab !== 'browser' ? <Button variant="primary" onClick={() => void SaveSettings()}>保存更改</Button> : undefined} />
