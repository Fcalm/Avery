        ...
        {selectedConversation ? <>
          <div className="trace-detail-header">
            <div className="trace-detail-metadata">
              ...
              <span className="trace-scene"><b>求职助手</b></span><button className="trace-detail-delete" type="button" aria-label={`删除${selectedConversation.title}的 Trace`} title="删除此对话的 Trace" onClick={() => { setDeleteError(null); setDeleteTarget(selectedConversation); }}><Icon name="delete" size={16} /></button>
            </div>
            <div className="trace-detail-controls"><div className="trace-search"><Icon name="search" size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索事件内容" aria-label="搜索事件内容" /></div><select value={eventFilter} onChange={(event) => setEventFilter(event.target.value as 'all' | TimelineEvent['title'])} aria-label="筛选事件"><option value="all">全部事件</option><option value="System">System</option><option value="User">User</option><option value="Tool">Tool</option><option value="Assistant">Assistant</option></select></div>
          </div>
          {visibleTimeline.length ? <div className="trace-event-list">{visibleTimeline.map((event, index) => <div key={event.id} className="trace-request-group">{(index === 0 || visibleTimeline[index - 1].requestId !== event.requestId) && <div className="trace-request-divider"><span>Request-{event.requestIndex}</span><code title={event.requestId}>{event.requestId}</code><button type="button" title="复制 Request ID" aria-label="复制 Request ID" onClick={() => CopyRequestId(event.requestId)}><Icon name="window-restore" size={12} /></button><small>{FormatTime(selectedConversation.traces[event.requestIndex - 1].createdAt)}</small></div>}<EventRow event={event} expanded={expanded.has(event.id)} onToggle={() => ToggleEvent(event.id)} /></div>)}</div> : <p className="empty-copy">此对话暂时没有可显示的 System、User、Tool 或 Assistant 事件。</p>}
