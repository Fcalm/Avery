...
function ResumesPage({ onGoAssistant }: { onGoAssistant: () => void }) {
  ...
  async function ExportResume(format: 'pdf' | 'docx' | 'png') {
    if (!editing) return;
    try {
  ...
  return <div className="standard-page"><PageHeader title="简历库" description="保留每一份针对目标岗位定制的简历版本。" actions={<Button variant="primary" onClick={onGoAssistant}><Icon name="assistant" size={16} />让 Agent 起草</Button>} />
    ...
    <Drawer open={drawerOpen} title="简历详情" onClose={() => setDrawerOpen(false)}>{editing && <div className="drawer-form"><FormField label="简历名称"><input value={rename} onChange={(event) => setRename(event.target.value)} /></FormField><Button onClick={SaveRename}>保存名称</Button><div className="resume-detail-preview"><pre>{editing.content}</pre></div><section className="resume-version-list"><header><b>版本历史</b><small>普通版本保留最近 100 个；重要版本永久保留</small></header>{revisions.length ? revisions.map((revision) => <div key={revision.id}><span>v{revision.revision} · {revision.source}</span><Button variant={revision.isPinned ? 'primary' : 'quiet'} onClick={() => void ToggleRevisionPinned(revision.id, revision.isPinned)}>{revision.isPinned ? '重要版本' : '标记重要'}</Button></div>) : <p>暂无可管理的历史版本。</p>}</section><div className="export-box"><span>导出文件</span><div><Button onClick={() => void ExportResume('pdf')}>PDF</Button><Button onClick={() => void ExportResume('docx')}>Word</Button><Button onClick={() => void ExportResume('png')}>图片</Button></div></div><div className="drawer-actions"><Button variant="danger" onClick={() => setDeleteTarget(editing)}>删除简历</Button><span /><Button variant="primary" onClick={() => { setCurrentResumeId(editing.id); ShowNotice('已设为当前简历'); }}>设为当前</Button></div></div>}</Drawer>
